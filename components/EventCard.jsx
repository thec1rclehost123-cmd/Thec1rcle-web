  ...
  return (
    ...
    >
      <Link href={`/event/${event.id || event.slug}`} className="group relative block h-full w-full">
        <div className={`gradient-border relative ${height} w-full overflow-hidden rounded-[32px] bg-white dark:bg-surface transition-all duration-500 btn-lift`}>
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-6">
            ...
            {/* Bottom Info */}
            <div className="absolute bottom-0 left-0 right-0 p-6 transform transition-transform duration-500 group-hover:translate-y-[-4px] z-20">
              {/* Glass Background on Hover */}
              <div className="absolute inset-0 -z-10 bg-black/40 backdrop-blur-xl opacity-0 group-hover:opacity-100 transition-all duration-500 rounded-t-[24px] translate-y-full group-hover:translate-y-0" />
              ...
              <div className="relative z-10">
                ...
                {event.guests && event.guests.length > 0 && (
                  <div className="mt-4 relative z-10">
                    <div className="group/guestlist inline-flex items-center gap-2.5 bg-white/10 hover:bg-white/15 border border-white/20 p-2 pr-4 rounded-full backdrop-blur-xl transition-all duration-300 hover:scale-[1.02]">
                      {/* Avatars Stack */}
                      <div className="flex -space-x-2.5">
                        {event.guests.slice(0, 4).map((guest, i) => (
                          <div
                            key={i}
                            className="relative h-7 w-7 rounded-full ring-2 ring-black/50 bg-gradient-to-br from-purple-400 to-pink-400 transition-all duration-300 group-hover/guestlist:ring-white/30"
                            style={{ zIndex: 4 - i }}
                          >
                            <img
                        ...
                        {event.guests.length > 4 && (
                          <div className="relative h-7 w-7 flex items-center justify-center rounded-full ring-2 ring-black/50 bg-gradient-to-br from-zinc-800 to-zinc-900 text-[9px] font-extrabold text-white z-0 group-hover/guestlist:ring-white/30 transition-all duration-300">
                            +{event.guests.length - 4}
                          </div>
                        )}
                      </div>
                ...
                {/* Hover Reveal Button */}
                <div className="mt-6 flex items-center gap-3 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 delay-75">
