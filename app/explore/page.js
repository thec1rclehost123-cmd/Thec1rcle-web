...
export default function ExplorePage() {
  ...
  const cityOptions = useMemo(() => {
    ...
    events.forEach((event) => {
      const label = event.city || event.location || "Pune, IN";
      const value = event.citySlug || slugify(label);
      if (!map.has(value)) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    const matchesCity = (event) => {
      ...
      const eventSlug = event.citySlug || slugify(event.city || event.location || "");
      return eventSlug === targetCity;
    };
